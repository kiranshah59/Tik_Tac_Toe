    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-dark-bg/80 backdrop-blur-xl border-b border-white/5 transition-all" role="navigation" aria-label="Main navigation">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-[72px] relative">
          ...
          <Link
            to="/"
            className="flex items-center gap-2.5 group focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-full"
            aria-label="Tic-Tac-Toe Home"
          >
            <div className="w-8 h-8 rounded-full bg-[#2A62FF] flex items-center justify-center font-bold text-white text-sm shadow-[0_0_15px_rgba(42,98,255,0.7)] transition-transform group-hover:scale-105">
              T³
            </div>
          ...
          <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center gap-6">
            {navLinks.map(({ path, label, icon: Icon }) => {
              ...
              return (
                ...
                >
                  <Icon size={16} className={isActive ? 'text-primary' : 'group-hover:text-primary-light transition-colors'} />
                  <span>{label}</span>
                  {isActive && (
                    <span className="absolute top-0 left-1/2 -translate-x-1/2 w-[80%] h-[1px] bg-gradient-to-r from-transparent via-[#4F46E5] to-transparent opacity-100 shadow-[0_0_10px_rgba(79,70,229,0.8)]" />
                  )}
                </Link>
              );
            })}
